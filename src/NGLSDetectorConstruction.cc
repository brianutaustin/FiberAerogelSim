...
G4ThreadLocal
G4GlobalMagFieldMessenger * NGLSDetectorConstruction::fMagFieldMessenger = 0;

NGLSDetectorConstruction::NGLSDetectorConstruction()
  ...
  fStepLimit(NULL),
  fCheckOverlaps(true) {
  fMessenger = new NGLSDetectorMessenger(this);
...
G4VPhysicalVolume * NGLSDetectorConstruction::Construct() {
  // Define materials
  DefineMaterials();
...
void NGLSDetectorConstruction::DefineMaterials() {
  // Material definition
...
G4VPhysicalVolume * NGLSDetectorConstruction::DefineVolumes() {
...
void NGLSDetectorConstruction::ConstructSDandField() {
  // Sensitive detectors

  G4String trackerChamberSDname = "NGLS/TrackerChamberSD";
  NGLSTrackerSD * aTrackerSD = new NGLSTrackerSD(trackerChamberSDname,
...
void NGLSDetectorConstruction::SetTargetMaterial(G4String materialName) {
  G4NistManager * nistManager = G4NistManager::Instance();
...
void NGLSDetectorConstruction::SetChamberMaterial(G4String materialName) {
  G4NistManager * nistManager = G4NistManager::Instance();
...
void NGLSDetectorConstruction::SetMaxStep(G4double maxStep) {
  if ((fStepLimit) && (maxStep > 0.)) {
    fStepLimit->SetMaxAllowedStep(maxStep);
  }
}

void NGLSDetectorConstruction::SetCheckOverlaps(G4bool checkOverlaps) {
